...
?>
   <div class="row sol_diagnostico">
     <div class="col-lg-5">
        ...
        class="form-control" name="SolicitudDiagnosticos[<?=$key?>][descripcion]"
     ...
     <div class="col-lg-2">
        ...
        class="form-control" name="SolicitudDiagnosticos[<?=$key?>][codigo]"
         value='<?=($sol_diagnostico->diagnostico)?$sol_diagnostico->diagnostico->codigo:'' ?>' readonly>
         <?=$form->field($sol_diagnostico, 'id_diagnostico')
         ->hiddenInput(['id' => "solicitudDiagnostico-id_diagnostico{$key}",'name' => "SolicitudDiagnosticos[$key][id_diagnostico]", 'value'=>2])
         ->label(false); ?>
     </div>
     <div class="col-lg-1">
       <?=$form->field($sol_diagnostico, 'principal')->checkBox([
       'id' => "solicitudDiagnostico-principal{$key}",
       'name' => "SolicitudDiagnosticos[$key][principal]",
         'class' =>'form-control',
         'label' =>false,
          // 'checked' => '1',
         // 'value' => '1',
     ...
     <div class="col-lg-1">
       <?=$form->field($sol_diagnostico, 'registro_usuario')->hiddenInput(['name' => "SolicitudDiagnosticos[$key][registro_usuario]", 'value'=> Yii::$app->user->identity->usuario])->label(false); ?>
       ...
       <i class="glyphicon glyphicon-search"></i></button>
    	...
    	<div class="col-lg-1">
        <?=$form->field($sol_diagnostico, 'registro_tiempo')->hiddenInput(['name' => "SolicitudDiagnosticos[$key][registro_tiempo]",'value' => date('Y-m-d H:i:s')])->label(false); ?>
